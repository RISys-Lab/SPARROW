...
def eval_davis(results_path, davis_path, task, set_name, debug=False):
    time_start = time()

    csv_name_global = f"global_results-{set_name}.csv"
    csv_name_per_sequence = f"per-sequence_results-{set_name}.csv"
    ...
    dataset_eval = DAVISEvaluation(davis_root=davis_path, task=task, gt_set=set_name)
    metrics_res = dataset_eval.evaluate(results_path, debug=debug)
    ...
    # Print the results
    sys.stdout.write(f"--------------------------- Global results for {set_name} ---------------------------\n")
    print(table_g.to_string(index=False))
    sys.stdout.write(f"\n---------- Per sequence results for {set_name} ----------\n")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate Refer-DAVIS predictions with DAVIS J/F metrics.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="vis_output/eval_davis17",
        help="Root directory that contains prediction folders (e.g., output_dir/valid/Davis17_annot1).",
    )
    parser.add_argument(
        "--split",
        type=str,
        default="valid",
        choices=["valid"],
        help="Prediction split directory inside output_dir.",
    )
    parser.add_argument(
        "--annotation_folders",
        nargs="+",
        default=[
            "Davis17_annot1",
            "Davis17_annot1_full_video",
            "Davis17_annot2",
            "Davis17_annot2_full_video",
        ],
        help="Prediction subfolders to evaluate under output_dir/split.",
    )
    parser.add_argument(
        "--davis_path",
        type=str,
        default="/media/ansari/BFEE-795F/VideoGLaMM/video_dataset/refer_davis/DAVIS17/DAVIS",
    ...
    parser.add_argument(
        ...
        help="DAVIS ground-truth set.",
    )
    parser.add_argument("--debug", action="store_true", help="Print per-sequence progress.")
    return parser.parse_args()


def main():
    args = parse_args()
    for folder in args.annotation_folders:
        results_path = os.path.join(args.output_dir, args.split, folder)
        eval_davis(
            results_path=results_path,
            davis_path=args.davis_path,
            task=args.task,
            set_name=args.gt_set,
            debug=args.debug,
        )
