...
class ConvGenerator_Base:
    ...
    DEFAULT_TSF_TOKEN = None
    NUM_FRAMES = None
    IGNORE_INDEX = None
    
    def __init__(self, use_mm_start_end, use_tsf_token=False):
        self.conversation_lib = None
        self.tokenizer_image_token = None
        
        self.use_mm_start_end = use_mm_start_end
        self.use_tsf_token = use_tsf_token

    @staticmethod
    def _inject_box_before_seg(text: str) -> str:
        """
        Convert each bare [SEG] token into [BOX] [SEG] while avoiding duplication.
        """
        if not isinstance(text, str):
            return text
        # do not duplicate if [BOX] already precedes [SEG] with optional whitespace
        return re.sub(r"(?<!\[BOX\]\s)\[SEG\]", "[BOX] [SEG]", text)

    def _build_tsf_placeholder(self, is_video: bool) -> str:
        if self.DEFAULT_TSF_TOKEN is None:
            return ""
        num_tokens = self.NUM_FRAMES if is_video else 1
        num_tokens = max(1, int(num_tokens))
        return self.DEFAULT_TSF_TOKEN * num_tokens
    
    
    def __preprocess_multimodal(self, source):

        for sentence in source:
            is_video_prompt = self.DEFAULT_VIDEO_TOKEN in sentence["value"]
            ...
            # Optional Target-Specific Feature placeholder; keep as raw token for
            # multimodal tokenizer to map to TSF token index.
            if (
                self.use_tsf_token
                and sentence.get("from", None) == "human"
                and self.DEFAULT_TSF_TOKEN is not None
                and self.DEFAULT_TSF_TOKEN not in sentence["value"]
            ):
                tsf_placeholder = self._build_tsf_placeholder(is_video=is_video_prompt)
                sentence["value"] = sentence["value"].strip() + "\n" + tsf_placeholder
    ...
    def apply(self, source):
        ...
        for j, sentence in enumerate(source):
            ...
            if sentence["from"] == "gpt":
                value = self._inject_box_before_seg(value)
    ...
    def apply_on_semseg_dataset(self, source):
        ...
        for i in range(len(source)):
            ...
            conv.append_message(conv.roles[1], self._inject_box_before_seg(source[i]['value']))
            conversations.append(conv.get_prompt())
            
        return conversations
    
    def apply_for_chat(self, prompt_text, type='video', tokenizer=None, include_tsf=None):
        ''' 
        Returns: 
        - input_ids: tensor of text prompt tokenized : (B x L)
        '''
        if include_tsf is None:
            include_tsf = self.use_tsf_token
        ...
        if type == 'video':
            ...
            if include_tsf and self.DEFAULT_TSF_TOKEN is not None:
                prompt = prompt + "\n" + self._build_tsf_placeholder(is_video=True)
        ...
        #  if Image
            ...
            if self.use_mm_start_end:
                ...
                prompt = prompt.replace(self.DEFAULT_IMAGE_TOKEN, replace_token)
            if include_tsf and self.DEFAULT_TSF_TOKEN is not None:
                prompt = prompt + "\n" + self._build_tsf_placeholder(is_video=False)
...
class ConvGenerator_VideoGPTPlus(ConvGenerator_Base):
    ...
    DEFAULT_TSF_TOKEN = constants_videogpt_plus.DEFAULT_TSF_TOKEN
    IGNORE_INDEX = constants_videogpt_plus.IGNORE_INDEX
    NUM_FRAMES = constants_videogpt_plus.NUM_FRAMES
    
    def __init__(self, use_mm_start_end, base_type='phi3', use_tsf_token=False):
        super().__init__(use_mm_start_end=use_mm_start_end, use_tsf_token=use_tsf_token)
