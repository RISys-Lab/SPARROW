...
if __name__=='__main__':
    parser = argparse.ArgumentParser(description="Preprocess VidSTG and VidOR annotations.")
    parser.add_argument(
        "--base_video_dataset_dir",
        default=os.getenv("VIDEO_DATASET_DIR", "datasets/video"),
        help="Root video dataset directory that contains vidstg/ and processed/.",
    )
    ...
    os.makedirs(vidstg_preprocessed_annotations_dir, exist_ok=True)
    ...
    for split in vidor_splits:
        ...
        for subdir in tqdm(subdirs):
            ...
            for file in files:
                with open(os.path.join(vidor_annotations_dir, split, subdir, file), "r") as f:
                    annot = json.load(f)
                out = {
        ...
        with open(os.path.join(vidstg_preprocessed_annotations_dir, "vidor_" + split + ".json"), "w") as f:
            json.dump(outs, f)
    print(categories)
    ...
    for file in files:
        ...
        trajectories = {}
        obj_categories = {}
        with open(os.path.join(vidstg_annotations_dir, file), "r") as f:
            annotations = json.load(f)
        vidor_filename = "vidor_training.json" if ("train" in file or "val" in file) else "vidor_validation.json"
        with open(os.path.join(vidstg_preprocessed_annotations_dir, vidor_filename), "r") as f:
            vidor = json.load(f)
        ...
        with open(os.path.join(vidstg_preprocessed_annotations_dir, file.split("_")[0] + ".json"), "w") as f:
            json.dump(outfile, f)
