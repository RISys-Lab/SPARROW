...
# -------------------- Base dataset for HC-STVG (A2D-style tree) --------------------

class HCSTVGSeg_BaseDataset(Dataset):
    """
    Traverses:
      root/
        Images/ or Val_Images/         # chosen via split={'train','val'}
          <sequence>/                  # e.g., seq1
            img/ mask/ groundtruth.txt nlp.txt
          <sequence>/<scene>/          # e.g., 00, 01, ...
            img/ mask/ groundtruth.txt nlp.txt
    ...
    def __init__(self, root_dir: str, split: str = "train"):
        ...
        split_dirname = "Images" if split == "train" else "Val_Images"
        self.images_root = self.root / split_dirname
        if not self.images_root.is_dir():
            raise FileNotFoundError(f"Images root not found for split '{split}': {self.images_root}")
    ...
    def _collect_items(self, images_root: Path):
        ...
        for seq_dir in sorted(p for p in images_root.iterdir() if p.is_dir()):
            if self._is_sample_dir(seq_dir):
                items.append(("HC-STVG", f"{seq_dir.name}", seq_dir))
                continue
            for scene_dir in sorted(p for p in seq_dir.iterdir() if p.is_dir()):
                if self._is_sample_dir(scene_dir):
                    items.append(("HC-STVG", f"{seq_dir.name}/{scene_dir.name}", scene_dir))
        return items
    ...
    def _mask_candidates(mask_dir: Path, frame_name: str, obj_id: str) -> List[Path]:
        ...
        return [
            mask_dir / zz / f"{stem}.png",
            mask_dir / f"{stem}_{obj_id}.png",
            mask_dir / f"{stem}.png",
            mask_dir / zz / f"{full}.png",
            mask_dir / f"{full}_{obj_id}.png",
            mask_dir / f"{full}.png",
        ]
    ...
    def _read_groundtruth_xyxy(gt_path: Path) -> List[Tuple[float, float, float, float]]:
        """
        Lines like 'x1,y1,x2,y2' or 'x1 y1 x2 y2' per frame.
        """
        out = []
...
class HCSTVGSeg_Dataset(Dataset):
    ignore_label = 255
    ...
    def __init__(
        ...
        self.base = HCSTVGSeg_BaseDataset(root_dir=base_video_dataset_dir, split=split)
        print(f"Done loading {len(self.base)} HC-STVG samples from split='{split}'.")
    ...
    def __getitem__(self, idx):
        ...
        _ = self.enc_preprocessor.preprocess(crop_pils)  # not exposed in normal dataset
