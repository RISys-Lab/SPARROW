...
from util.utils import (AverageMeter, ProgressMeter, Summary, dict_to_cuda,intersectionAndUnionGPU)
...
class LISATrainer:
    ...
    def __train_epoch(self, train_loader, model, epoch, scheduler, writer, train_iter, args):
        ...
        proposal_losses = AverageMeter("ProposalLoss", ":.4f")
        weighted_proposal_losses = AverageMeter("WeightedProposalLoss", ":.4f")
        proposal_box_losses = AverageMeter("ProposalBoxLoss", ":.4f")
        weighted_proposal_box_losses = AverageMeter("WeightedProposalBoxLoss", ":.4f")
        ...
        progress = ProgressMeter(
            ...
            [
                ...
                mask_dice_losses,
                proposal_losses,
                weighted_proposal_losses,
                proposal_box_losses,
                weighted_proposal_box_losses,
            ],
        ...
        for global_step in tqdm(range(args.steps_per_epoch)):
            for i in range(args.grad_accumulation_steps):
                ...
                if args.precision == "fp16":
                    ...
                    if "tsf_images" in input_dict:
                        input_dict["tsf_images"] = [a.half() if a is not None else None for a in input_dict["tsf_images"]]
                    ...
                    if "tsf_images" in input_dict:
                        input_dict["tsf_images"] = [a.bfloat16() if a is not None else None for a in input_dict["tsf_images"]]
                else:
                    ...
                    if "tsf_images" in input_dict:
                        input_dict["tsf_images"] = [a.float() if a is not None else None for a in input_dict["tsf_images"]]
                ...
                proposal_loss = output_dict.get("proposal_loss", loss.new_tensor(0.0))
                if not torch.is_tensor(proposal_loss):
                    proposal_loss = loss.new_tensor(float(proposal_loss))
                weighted_proposal_loss = proposal_loss * getattr(args, "proposal_loss_weight", 1.0)
                proposal_box_loss = output_dict.get("proposal_box_loss", loss.new_tensor(0.0))
                if not torch.is_tensor(proposal_box_loss):
                    proposal_box_loss = loss.new_tensor(float(proposal_box_loss))
                weighted_proposal_box_loss = proposal_box_loss * getattr(args, "proposal_box_loss_weight", 0.0)
                ...
                proposal_losses.update(proposal_loss.item(), batch_size)
                weighted_proposal_losses.update(weighted_proposal_loss.item(), batch_size)
                proposal_box_losses.update(proposal_box_loss.item(), batch_size)
                weighted_proposal_box_losses.update(weighted_proposal_box_loss.item(), batch_size)
                model.backward(loss)
            ...
            if global_step % args.print_freq == 0:
                if args.distributed:
                    ...
                    proposal_losses.all_reduce()
                    weighted_proposal_losses.all_reduce()
                    proposal_box_losses.all_reduce()
                    weighted_proposal_box_losses.all_reduce()
                ...
                if args.local_rank == 0:
                    ...
                    writer.add_scalar("train/proposal_loss", proposal_losses.avg, global_step_tb)
                    writer.add_scalar(
                        "train/weighted_proposal_loss", weighted_proposal_losses.avg, global_step_tb
                    )
                    writer.add_scalar("train/proposal_box_loss", proposal_box_losses.avg, global_step_tb)
                    writer.add_scalar(
                        "train/weighted_proposal_box_loss", weighted_proposal_box_losses.avg, global_step_tb
                    )
                    writer.add_scalar(
                ...
                mask_losses.reset()
                proposal_losses.reset()
                weighted_proposal_losses.reset()
                proposal_box_losses.reset()
                weighted_proposal_box_losses.reset()

            if global_step != 0 and hasattr(scheduler, "get_last_lr"):
                curr_lr = scheduler.get_last_lr()
                if args.local_rank == 0 and len(curr_lr) > 0:
                    global_step_tb = epoch * args.steps_per_epoch + global_step
    ...
    def train(self, epochs):            
        ...
        for epoch in range(self.start_epoch, epochs):
            ...
            self._save_latest_only_checkpoint(epoch)
            # =====================================================================================

    

    def _save_latest_only_checkpoint(self, epoch):
        save_dir = self.ckpt_save_dir

        # rank 0 writes/updates lightweight training state
        if self.args.local_rank == 0:
            os.makedirs(save_dir, exist_ok=True)
            torch.save({"epoch": epoch}, os.path.join(save_dir, "trainer_state.pth"))

        torch.distributed.barrier()

        # DeepSpeed saves a new checkpoint, usually under save_dir/global_stepXXXX
        self.model_engine.save_checkpoint(save_dir)

        torch.distributed.barrier()

        # rank 0 removes older DeepSpeed checkpoint folders
        if self.args.local_rank == 0:
