...
# -------------------- LaSOT base dataset (no splits) --------------------

class LaSOTSeg_BaseDataset(Dataset):
    """
    Traverses LaSOT like:

      LaSOT/
        class/               (optional wrapper dir)
          <class-name>/
            <sequence>/                  (OR <sequence>/00, 01,... scenes)
              img/
              mask/
              groundtruth.txt
              nlp.txt

    If scenes (00/01/..) exist, each is one sample; otherwise the sequence root is the sample.

    Returns:
      class_id, seq_or_scene_id, pil_images, all_masks_dict, new_caption, phrases, boxes_xyxy_pixels
    """
    def __init__(self, root_dir: str):
        ...
        # accept LaSOT/class/* or LaSOT/* as top-level classes
        top_candidates = [self.root / "class", self.root]
        self.top = None
        for cand in top_candidates:
            if cand.is_dir() and any(d.is_dir() for d in cand.iterdir()):
                self.top = cand
                break
        if self.top is None:
            raise FileNotFoundError(f"No class directories found under {self.root}")

        self.items = self._collect_items(self.top)
        if not self.items:
            raise RuntimeError(f"No valid sequences found under {self.top}")

    @staticmethod
    def _is_sample_dir(d: Path) -> bool:
        return (d / "img").is_dir() and (d / "mask").is_dir() and (d / "groundtruth.txt").is_file() and (d / "nlp.txt").is_file()

    def _collect_items(self, top: Path):
        items = []
        for class_dir in sorted(p for p in top.iterdir() if p.is_dir()):
            for seq_dir in sorted(p for p in class_dir.iterdir() if p.is_dir()):
                # either seq_dir itself is a sample, or it contains scene subfolders that are samples
                if self._is_sample_dir(seq_dir):
                    items.append((class_dir.name, seq_dir.name, seq_dir))
                else:
                    # look for scenes like 00/01/... that are sample dirs
                    for scene_dir in sorted(p for p in seq_dir.iterdir() if p.is_dir()):
                        if self._is_sample_dir(scene_dir):
                            items.append((class_dir.name, f"{seq_dir.name}/{scene_dir.name}", scene_dir))
        return items
    ...
    def __getitem__(self, idx):
        class_id, sample_id, sample_dir = self.items[idx]
        ...
        # boxes (xywh -> xyxy pixels), one per frame
        gt_xywh = self._read_groundtruth_xywh(gt_path)
        if len(gt_xywh) != len(pil_images):
            raise AssertionError(f"groundtruth length ({len(gt_xywh)}) != #frames ({len(pil_images)}) at {sample_dir}")
        boxes_xyxy_pixels = [self._xywh_to_xyxy_pixels(*b) for b in gt_xywh]

        # masks
        all_masks = self._build_masks(mask_dir, frame_names, obj_ids_list)

        return class_id, sample_id, pil_images, all_masks, new_caption, phrases or ["object"] * len(all_masks), boxes_xyxy_pixels


# -------------------- Wrapper: adds normalized boxes + images4box --------------------

class LaSOTSeg_Dataset(Dataset):
    ignore_label = 255
    ...
    def __init__(
        ...
        self.base = LaSOTSeg_BaseDataset(root_dir=base_video_dataset_dir)
        print(f"Done loading {len(self.base)} LaSOT samples.")
    ...
    def __getitem__(self, idx):
        # base returns: class_id, sample_id, pil_images, all_masks_dict, caption, phrases, boxes_xyxy_pix
        ...
        # --------- images4box (crops through enc_preprocessor) ----------
        crop_pils = []
        boxes_norm = []
        ...
        preprocessed_crops = self.enc_preprocessor.preprocess(crop_pils)
