...
# -------------------- Base dataset for ILSVRC VID-style tree --------------------

class VID2015Seg_BaseDataset(Dataset):
    """
    Traverses:
      root/
        {train,val}/
          ILSVRC2015_VID_train_0000/   # "bucket" dirs (or similar)
    ...
    Returns:
      bucket_id, sample_id, pil_images, all_masks_dict, new_caption, phrases, boxes_xyxy_pixels
    """
    def __init__(self, root_dir: str, split: str = "train"):
        ...
        self.items = self._collect_items(split_dir)
        if not self.items:
            raise RuntimeError(f"No valid samples under {split_dir}")
    ...
    def _collect_items(self, split_dir: Path):
        items = []
        # first-level buckets (e.g., ILSVRC2015_VID_train_0000)
        for bucket_dir in sorted(p for p in split_dir.iterdir() if p.is_dir()):
            # sequences under bucket
            for seq_dir in sorted(p for p in bucket_dir.iterdir() if p.is_dir()):
                if self._is_sample_dir(seq_dir):
                    items.append((bucket_dir.name, seq_dir.name, seq_dir))
                else:
                    # try scenes (00/01/..)
                    for scene_dir in sorted(p for p in seq_dir.iterdir() if p.is_dir()):
                        if self._is_sample_dir(scene_dir):
                            ...
                            items.append((bucket_dir.name, sample_id, scene_dir))
        return items
    ...
    def _read_groundtruth_xyxy(gt_path: Path) -> List[Tuple[float, float, float, float]]:
        """
        Parses lines like: 'x1,y1,x2,y2' or 'x1 y1 x2 y2'. Ignores blanks.
        Returns list of (x1,y1,x2,y2) as floats per frame.
        """
        out = []
    ...
    def __getitem__(self, idx):
        bucket_id, sample_id, sample_dir = self.items[idx]
        ...
        return bucket_id, sample_id, pil_images, all_masks, new_caption, phrases or ["object"] * len(all_masks), boxes_xyxy_pixels


# -------------------- Wrapper: normalized boxes + images4box, SAM, etc. --------------------

class VID2015Seg_Dataset(Dataset):
    ignore_label = 255
    ...
    def __init__(
        ...
        self.base = VID2015Seg_BaseDataset(root_dir=base_video_dataset_dir, split=split)
        print(f"Done loading {len(self.base)} VID2015 samples from split='{split}'.")
    ...
    def __getitem__(self, idx):
        # base returns: bucket_id, sample_id, pil_images, all_masks_dict, caption, phrases, boxes_xyxy_pix
