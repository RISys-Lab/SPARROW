...
class VidSTGDataset(torch.utils.data.Dataset):
    ...
    def get_from_idx(self, idx):
        ...
        # Keep masks aligned with the selected temporal window.
        gt_masks_window = gt_masks[time_s:time_e+1]

        # Remove indices where gt_mask is None
        indices = [i for i, mask in enumerate(gt_masks_window) if mask is not None]
        np_images_for_sam = np_images_for_sam[indices]
        gt_masks = [mask for mask in gt_masks_window if mask is not None]
        gt_masks = np.array(gt_masks)

        if len(enc_out['images']) == 0:
            raise ValueError(f"No encoder frames for idx={idx}, time window=({time_s},{time_e}).")
        if len(np_images_for_sam) == 0 or len(gt_masks) == 0:
            raise ValueError(f"No valid SAM frames/masks for idx={idx}, time window=({time_s},{time_e}).")
    ...
    def __getitem__(self, idx):
        ...
        raise RuntimeError(f"VIDSTGDataset failed after {max_retries} retries. Last error: {last_err}")
