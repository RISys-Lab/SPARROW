...
from sparrow.train.dataset_samplers import EqualizedDatasetBatchSampler
...
class GromaTrainer(Trainer):
    def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
        ...
        if self.args.group_by_data_source:
            ...
            if getattr(self.args, "equalize_data_sources", False):
                samples_per_dataset = int(getattr(self.args, "samples_per_dataset", 0))
                if samples_per_dataset <= 0:
                    raise ValueError(
                        "equalize_data_sources=True requires samples_per_dataset > 0."
                    )
                return EqualizedDatasetBatchSampler(
                    self.train_dataset,
