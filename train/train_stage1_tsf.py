"""Stage-1 training script (TSF injection).

This stage optimizes LoRA adapters in the multimodal adapters and LLM while
keeping the visual backbones and pixel decoders frozen.
"""
...
class AllArguments:
    ...
    use_tsf_token:bool=field(default=True, metadata={"help": "Enable Target-Specific Features (TSF) token and feature injection."})
    ...
    tune_mm_mlp_adapter:bool=field(default=False, metadata={"help": "Legacy flag kept for compatibility; Stage-1 keeps base adapters frozen and trains LoRA params."})
    ...
    epochs:int=field(default=30, metadata={"help": "Number of training epochs."})
    ...
    # =========================
    use_detection_guidance:bool=field(default=False, metadata={"help": "Stage-1 keeps DDETR proposal guidance disabled."})
...
def main():
    ...
    # Stage-1 constraints:
    # - TSF injection is enabled.
    # - Detection guidance is disabled (CE + mask losses only).
    if not args.use_tsf_token:
        print("[Stage1-TSF] Overriding --use_tsf_token=False to True.")
        args.use_tsf_token = True
    if args.use_detection_guidance:
        print("[Stage1-TSF] Overriding --use_detection_guidance=True to False.")
        args.use_detection_guidance = False
    ...
    if not (args.eval_only or args.save_hf_model):
        ...
        # Make Video encoder frozen
        for p in image_vision_tower.parameters(): #NOTE
            p.requires_grad = False
        

        # Keep base adapters frozen; LoRA adapters will be trainable.
    ...
    # Stage-1 keeps detection guidance and proposal heads disabled/frozen.
