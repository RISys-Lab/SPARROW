...
def _kmeans_select_representatives(x: torch.Tensor, k: int = 4, num_iters: int = 8) -> torch.Tensor:
    """
    Select k representative vectors from x (N, D) using lightweight k-means+medoid.
    Returns (min(k, N), D).
    """
    ...
    x32 = x.float()
    init_idx = torch.linspace(0, n - 1, steps=k, device=x.device).round().long()
    centers = x32[init_idx]

    for _ in range(max(1, int(num_iters))):
        dist = torch.cdist(x32, centers, p=2)  # (N, K)
        assign = dist.argmin(dim=1)  # (N,)
        new_centers = []
        for cid in range(k):
            member_mask = assign == cid
            if member_mask.any():
                new_centers.append(x32[member_mask].mean(dim=0))
            else:
    ...
    # pick nearest sample (medoid-like) per cluster center to keep values on-data.
    center_to_sample = torch.cdist(centers, x32, p=2).argmin(dim=1)  # (K,)
    return x[center_to_sample]
...
class VideoGPTPlusMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, attention_mask, past_key_values, labels, 
        ...
        if tsf_images is None:
            tsf_images = [None] * len(images)
        
        seg_token_mask = input_ids[:, 1:] == self.config.seg_token_idx
        seg_token_mask = torch.cat(
            [seg_token_mask, torch.zeros((seg_token_mask.shape[0], 1), dtype=torch.bool, device=input_ids.device)],
            dim=1,
        )
        box_token_idx = getattr(self.config, "box_token_idx", None)
        if box_token_idx is None:
            box_token_mask = torch.zeros_like(seg_token_mask, dtype=torch.bool)
        else:
            box_token_mask = input_ids[:, 1:] == box_token_idx
            box_token_mask = torch.cat(
                [box_token_mask, torch.zeros((box_token_mask.shape[0], 1), dtype=torch.bool, device=input_ids.device)],
                dim=1,
            )
        ...
        if (vision_tower is None and image_vision_tower is None) or images is None or input_ids.shape[1] == 1:
            ...
            return input_ids, attention_mask, past_key_values, None, labels, seg_token_mask, box_token_mask
        ...
        for batch_idx, (images_batch, context_images_batch) in enumerate(zip(images, context_images)):
            ...
            tsf_batch = tsf_images[batch_idx] if batch_idx < len(tsf_images) else None
            if tsf_batch is None:
                tsf_features[batch_idx] = None
            else:
                if tsf_batch.ndim == 3:
                    tsf_batch = tsf_batch.unsqueeze(0)
                tsf_enc = self.encode_images(tsf_batch)  # [N_tsf, L, D]
                tsf_proj = self.project(video_features=None, context_features=tsf_enc, input_type="image")
                tsf_frame_tokens = tsf_proj.mean(dim=1)  # [N_tsf, D]
                tsf_k = int(getattr(self.config, "tsf_k", 4))
                tsf_kmeans_iters = int(getattr(self.config, "tsf_kmeans_iters", 8))
                tsf_selected = _kmeans_select_representatives(
                    tsf_frame_tokens,
                    k=tsf_k,
                    num_iters=tsf_kmeans_iters,
                )  # [K, D]
                tsf_features[batch_idx] = [tsf_selected[i:i + 1] for i in range(tsf_selected.shape[0])]
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if (cur_input_ids == IMAGE_TOKEN_INDEX).sum() == 0:
                ...
                new_box_token_masks.append(box_token_mask[batch_idx])
                continue
            ...
            cur_box_token_mask = box_token_mask[batch_idx]
            cur_new_box_token_masks = []
            ...
            if len(image_token_indices) > 1:  # This is a video
                ...
                for i in temp:
                    ...
                    if getattr(self.config, 'tune_mm_mlp_adapter', False) and getattr(
                        ...
                        cur_new_box_token_masks.append(cur_box_token_mask[:image_token_start])
                        cur_new_box_token_masks.append(torch.full((cur_image_features.shape[0],), 0, device=cur_image_features.device, dtype=input_ids.dtype))
                        cur_new_box_token_masks.append(cur_box_token_mask[image_token_end:image_token_end + 1])
                        cur_box_token_mask = cur_box_token_mask[image_token_end + 2:]
                    ...
                    # if start/end tokens are not used
                        ...
                        cur_new_box_token_masks.append(cur_box_token_mask[:image_token_start])
                        cur_new_box_token_masks.append(torch.full((cur_image_features.shape[0],), 0, device=cur_image_features.device, dtype=input_ids.dtype))
                        cur_box_token_mask = cur_box_token_mask[image_token_end + 1:]
            ...
            # Optional TSF token insertion after image/video token handling.
            tsf_token_indices = torch.where(cur_input_ids == TSF_TOKEN_INDEX)[0]
            if tsf_token_indices.numel() > 0:
                tsf_blocks = tsf_features.get(batch_idx, None)
                tsf_k = int(getattr(self.config, "tsf_k", 4))
                if tsf_blocks is not None and len(tsf_blocks) > 0:
                    max_keep = min(tsf_k, len(tsf_blocks), int(tsf_token_indices.numel()))
                else:
                    max_keep = min(tsf_k, int(tsf_token_indices.numel()))
                prev_idx = 0
                embed_dtype = self.get_model().embed_tokens.weight.dtype
                embed_dim = self.get_model().embed_tokens.weight.shape[1]

                for idx_order, tsf_pos_tensor in enumerate(tsf_token_indices):
                    tsf_pos = int(tsf_pos_tensor.item())

                    if tsf_pos > prev_idx:
                        cur_new_input_embeds.append(self.get_model().embed_tokens(cur_input_ids[prev_idx:tsf_pos]))
                        if labels is not None:
                            cur_new_labels.append(cur_labels[prev_idx:tsf_pos])
                        cur_new_seg_token_masks.append(cur_seg_token_mask[prev_idx:tsf_pos])
                        cur_new_box_token_masks.append(cur_box_token_mask[prev_idx:tsf_pos])

                    if idx_order < max_keep:
                        if tsf_blocks is not None and len(tsf_blocks) > 0:
                            block_idx = min(idx_order, len(tsf_blocks) - 1)
                            tsf_embed = tsf_blocks[block_idx].to(device=self.device)
                        else:
                            tsf_embed = torch.zeros((1, embed_dim), device=self.device, dtype=embed_dtype)

                        cur_new_input_embeds.append(tsf_embed)
                        if labels is not None:
                            cur_new_labels.append(
                                torch.full(
                                    (tsf_embed.shape[0],),
                        ...
                        cur_new_seg_token_masks.append(
                            torch.zeros((tsf_embed.shape[0],), device=tsf_embed.device, dtype=input_ids.dtype)
                        )
                        cur_new_box_token_masks.append(
                            torch.zeros((tsf_embed.shape[0],), device=tsf_embed.device, dtype=input_ids.dtype)
                        )
                    prev_idx = tsf_pos + 1

                cur_input_ids = cur_input_ids[prev_idx:]
                if labels is not None:
                    cur_labels = cur_labels[prev_idx:]
                cur_seg_token_mask = cur_seg_token_mask[prev_idx:]
                cur_box_token_mask = cur_box_token_mask[prev_idx:]
            ...
            if cur_input_ids.numel() > 0:
                ...
                cur_new_box_token_masks.append(cur_box_token_mask)
            ...
            cur_new_box_token_masks = torch.cat(cur_new_box_token_masks, dim=0)
            new_box_token_masks.append(cur_new_box_token_masks)
        ...
        if any(x.shape != new_input_embeds[0].shape for x in new_input_embeds):
            ...
            new_box_token_masks_align = []
            for cur_new_box_token_mask in new_box_token_masks:
                cur_new_box_token_mask = torch.cat(
                    (
                        cur_new_box_token_mask,
                        torch.full(
                            (max_len - cur_new_box_token_mask.shape[0],),
                            0,
                            dtype=cur_new_box_token_mask.dtype,
                            device=cur_new_box_token_mask.device,
                        ),
                    ),
                    dim=0,
                )
                new_box_token_masks_align.append(cur_new_box_token_mask)
            new_box_token_masks = torch.stack(new_box_token_masks_align, dim=0).bool()
        ...
        # If all the samples in the batch are of the same length
            ...
            new_box_token_masks = torch.stack(new_box_token_masks, dim=0).bool()
        ...
        return None, attention_mask, past_key_values, new_input_embeds, new_labels, new_seg_token_masks, new_box_token_masks
